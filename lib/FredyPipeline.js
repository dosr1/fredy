...
import { saveRawPictures } from './services/storage/listingsRaw.js';
...
class FredyPipeline {
  ...
  /**
   * Persist raw pictures referenced by the new listings into listings_raw.
   * Uses the project's saveRawPictures helper to download & store image blobs.
   *
  ...
  async _savePictures(newListings) {
    ...
    const tasks = newListings.map(async (listing) => {
      ...
      if (!pics || !Array.isArray(pics) || pics.length === 0) return;

      // listings.id in the normalized object is the stable hash stored in the
      // `hash` column of the listings table. The listings_raw table has a
      // FOREIGN KEY referencing listings(id) (the primary key generated by
      // storeListings). We must resolve the real PK before inserting raw rows.
      try {
        ...
        if (!listingRowId) {
          logger.debug(`Could not find persisted listing row for hash ${listing.id}; skipping raw pictures`);
          return;
        }

        await saveRawPictures(listingRowId, pics);
      } catch (e) {
        logger.debug(`saveRawPictures failed for listing ${listing.id}: ${e?.message ?? e}`);
      }
    });
