...
import axios from 'axios'; // Für das Herunterladen der Bilder
...
class FredyPipeline {
  ...
  execute() {
    return Promise.resolve(urlModifier(this._providerConfig.url, this._providerConfig.sortByDateParam))
      ...
      .then(this._savePictures.bind(this))
  ...
  /**
   * Persist raw pictures and additional documents referenced by the new listings into the data/<listing_id> folder.
   *
   * @param {Listing[]} newListings
   * @returns {Promise<Listing[]>} resolves with the same listings
   */
  async _savePictures(newListings) {
    if (!Array.isArray(newListings) || newListings.length === 0) return newListings;

    const tasks = newListings.map(async (listing) => {
      const pics =
        listing.additionalpictures ||
        listing.additional_pictures ||
        listing.pictures ||
        listing.images ||
        (listing.meta && (listing.meta.additionalPictures || listing.meta.additional_pictures || listing.meta.images));

      const docs =
        listing.additionaldocuments ||
        listing.additional_documents ||
        (listing.meta && listing.meta.additionalDocuments);

      logger.info(`raw pics:`, pics);
      logger.info(`raw docs:`, docs);

      if ((!pics || !Array.isArray(pics) || pics.length === 0) && (!docs || !Array.isArray(docs) || docs.length === 0))
        return;

      try {
        // Hole die ID des Listings aus der Datenbank
        const rows = SqliteConnection.query(
          `SELECT id FROM listings WHERE hash = @hash AND job_id = @jobId AND provider = @provider LIMIT 1`,
          { hash: listing.id, jobId: this._jobKey, provider: this._providerId },
        );
        const listingRowId = rows?.[0]?.id;
        if (!listingRowId) {
          logger.debug(
            `Could not find persisted listing row for hash ${listing.id}; skipping raw pictures and documents`,
          );
          return;
        }

        // Erstelle den Zielordner für die Bilder und Dokumente
        const targetDir = path.join('data', listingRowId.toString());
        if (!fs.existsSync(targetDir)) {
          fs.mkdirSync(targetDir, { recursive: true });
        }

        // Lade und speichere jedes Bild
        if (pics && Array.isArray(pics)) {
          await Promise.all(
            pics.map(async (picUrl, index) => {
              try {
                const response = await axios.get(picUrl, { responseType: 'arraybuffer' });
                const filePath = path.join(targetDir, `image_${index + 1}.jpg`);
                fs.writeFileSync(filePath, response.data);
                logger.info(`Saved image to ${filePath}`);
              } catch (e) {
                logger.debug(`Failed to save image from ${picUrl}: ${e.message}`);
              }
            }),
          );
        }

        // Lade und speichere jedes Dokument
        if (docs && Array.isArray(docs)) {
          await Promise.all(
            docs.map(async (docUrl, index) => {
              try {
                const response = await axios.get(docUrl, { responseType: 'arraybuffer' });
                const filePath = path.join(targetDir, `document_${index + 1}.pdf`);
                fs.writeFileSync(filePath, response.data);
                logger.info(`Saved document to ${filePath}`);
              } catch (e) {
                logger.debug(`Failed to save document from ${docUrl}: ${e.message}`);
              }
            }),
          );
        }
      } catch (e) {
        logger.debug(`Failed to process pictures or documents for listing ${listing.id}: ${e.message}`);
      }
    });
