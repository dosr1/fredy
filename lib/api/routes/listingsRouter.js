...
// Return raw image entries for a given listing (listings_raw rows).
...
listingsRouter.post('/raw', async (req, res) => {
  try {
    ...
    const rows = SqliteConnection.query(
      `SELECT id, listing_id, link, mime_type, rawdata
       FROM listings_raw
    ...
    const out = rows.map((r) => ({
      ...
      rawdata: r.rawdata ? Buffer.from(r.rawdata).toString('base64') : null,
    }));
