  ...
  SqliteConnection.withTransaction((db) => {
    const stmt = db.prepare(
      `INSERT INTO listings (id, hash, provider, job_id, price, size, title, image_url, description, address,
           link, detailedinfo, additionalpictures, additionaldocuments, created_at, is_active)
       VALUES (@id, @hash, @provider, @job_id, @price, @size, @title, @image_url, @description, @address, @link,
         @detailedinfo, @additionalpictures, @additionaldocuments, @created_at, 1)
      ...
      const params = {
        ...
        additionalpictures:
          item.additionalpictures != null
            ...
            : null,
        // additionaldocuments: store as JSON string when array/object, or null
        additionaldocuments:
          item.additionaldocuments != null
            ? typeof item.additionaldocuments === 'string'
              ? item.additionaldocuments
              : JSON.stringify(item.additionaldocuments)
            : null,
...
export const queryListings = ({
  ...
  const parsed = rows.map((r) => {
    ...
    try {
      if (copy.additionaldocuments && typeof copy.additionaldocuments === 'string') {
        copy.additionaldocuments = JSON.parse(copy.additionaldocuments);
      }
    } catch {
      // leave as string if parsing fails
    }
